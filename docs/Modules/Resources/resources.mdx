...
Use `lid_from_jid` and `jid_from_lid` when a module needs to consolidate
WhatsApp phone-number JIDs and LID JIDs.
...
`response.lid` is the known `@lid` value or `null`.
...
`response.jid` is the known `@s.whatsapp.net` value or `null`.
