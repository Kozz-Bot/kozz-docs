...
The modules can request information from each other using an API called `Resource asking`.
...
The Kozz-Whatsapp boundary is prepared to provide some resources. But you can also insert resources into yout modules and have them exchanging data with each other.
...
setTimeout(() => {
  myOtherModule.ask
    .handler("MyModule", "Double", {
      //send argument
      number: 10,
    })
    .then(({ response }) => {
      // response is "The double of 10 is 20"
      myOtherModule.sendMessage(contactId, "Gramonta-Wa", response);
    });
}, 1000);
```
